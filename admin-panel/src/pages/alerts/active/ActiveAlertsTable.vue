...
const computedHoveredAlert = computed(() => {
  const alert = props.sortedData.find(a => a.id === hoveredId.value);
  ...
  <Table id="alert-table" :class="dataTable">
    ...
    <TableHeader class="h-10">
      <TableRow :class="tableHeaders">
        <SortableHead keyName="subject" label="Alert" :sort-key="sortKey" class="w-fit *:pl-2" :sort-order="sortOrder" @update:toggle-sort="$emit('update:toggleSort', $event)"/>
        <SortableHead keyName="severity" label="Severity" :sort-key="sortKey" class="max-md:w-1/7 max-lg:w-1/10 w-7/100 " :sort-order="sortOrder" @update:toggle-sort="$emit('update:toggleSort', $event)"/>
        <SortableHead keyName="message" label="Message" :sort-key="sortKey" class="w-fit md:min-w-1/5" :sort-order="sortOrder" @update:toggle-sort="$emit('update:toggleSort', $event)"/>
        <SortableHead keyName="source" label="Source" :sort-key="sortKey" class="w-fit " :sort-order="sortOrder" @update:toggle-sort="$emit('update:toggleSort', $event)"/>
        <SortableHead keyName="originType" label="Origin" :sort-key="sortKey" class="w-1/10 " :sort-order="sortOrder" @update:toggle-sort="$emit('update:toggleSort', $event)"/>
        <SortableHead keyName="isAcknowledged" label="ACK" :sort-key="sortKey" class="w-6/100 " :sort-order="sortOrder" @update:toggle-sort="$emit('update:toggleSort', $event)"/>
        <SortableHead keyName="createdAt" label="Created at" :sort-key="sortKey" class="w-fit md:w-15/100 lg:w-1/9" :sort-order="sortOrder" @update:toggle-sort="$emit('update:toggleSort', $event)"/>
